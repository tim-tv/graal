...
class ServiceSecureServerController {
    ...
    fun generateToken(@RequestHeader headers: HttpHeaders): ResponseEntity<AccessTokenResponse> {
        ...
        logger.info("[generateToken] => authenticating for ${details.getAppId()}:${details.getAppSecret()}")
        ...
        return ResponseEntity.ok(AccessTokenResponse(token.getValue(), expiresIn))
    }
